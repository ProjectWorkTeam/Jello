...
# def get_boards():
#     boards = Board.query.filter_by(owner_id=current_user.id).order_by(asc(Board.position_id)).all()
#     boards_data = [{'id': board.id, 'name': board.name, 'owner_id': board.owner_id, 'background': board.background, 'position_id': board.position_id} for board in boards]
#     return generate_success_response({'boards': boards_data})
...
#         new_board = Board(name=form.data["name"], owner_id=current_user.id, position_id=next_position_id, background=form.data["background"])
#         db.session.add(new_board)
#         db.session.commit()
...
# # Update a board's name, position, and/or background
# @board_routes.route('/<int:board_id>', methods=["PUT"])
# @login_required
# def update_board(board_id):
#     board = Board.query.get(board_id)
...
#     # Check if new background is provided
